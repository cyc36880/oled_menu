...
{
	const uint16_t MedianVoltage = 1920;

	static enum MenuState adcstate = Menu_noaction;
		
	if(ADCarray[0] > (MedianVoltage + 1500)){
		adcstate = Menu_Sub;
		return adcstate;
	}
	else if(ADCarray[0]<(MedianVoltage + 200) && ADCarray[0]>(MedianVoltage - 200)) adcstate = Menu_noaction;
	else if(ADCarray[0] < (MedianVoltage - 1500)) {
		adcstate = Menu_Father;
		return adcstate;
	}
	
	if(ADCarray[1] > (MedianVoltage + 1500)){
		adcstate = Menu_down;
		return adcstate;
	} 
	else if(ADCarray[1]<(MedianVoltage + 200) && ADCarray[1]>(MedianVoltage - 200)) adcstate = Menu_noaction;
	else if(ADCarray[1] < (MedianVoltage - 1500)){
		adcstate = Menu_up;
		return adcstate;
	} 
